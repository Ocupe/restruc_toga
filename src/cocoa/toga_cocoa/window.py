...
class WindowDelegate(NSObject):
    ...
    def windowWillClose_(self, notification) -> None:
        self._interface.creator.on_close()

    @objc_method
    def windowDidResize_(self, notification) -> None:
        if self._interface.creator.content:
            # print()
            ...
            if notification.object.contentView.frame.size.width > 0.0 and notification.object.contentView.frame.size.height > 0.0:
                ...
                self._interface.creator.content._update_layout(
...
class Window():
    ...
    def create(self):
        ...
        position = NSMakeRect(
            screen.origin.x + self.creator.position[0],
            screen.size.height + screen.origin.y - self.creator.position[1] - self.creator._size[1],
            self.creator._size[0],
            self.creator._size[1]
        )

        mask = NSTitledWindowMask
        if self.creator.closeable:
            mask |= NSClosableWindowMask

        if self.creator.resizeable:
            mask |= NSResizableWindowMask

        if self.creator.minimizable:
    ...
    def _set_content(self, widget):
        self._impl.setContentView_(self.creator._container._impl._impl)
        ...
        self._min_width_constraint = NSLayoutConstraint.constraintWithItem_attribute_relatedBy_toItem_attribute_multiplier_constant_(
            self.creator._container._impl._impl, NSLayoutAttributeRight,
            NSLayoutRelationGreaterThanOrEqual if self.creator.resizeable else NSLayoutRelationEqual,
            self.creator._container._impl._impl, NSLayoutAttributeLeft,
            1.0, 0
        )
        self.creator._container._impl._impl.addConstraint_(self._min_width_constraint)
        ...
        self._min_height_constraint = NSLayoutConstraint.constraintWithItem_attribute_relatedBy_toItem_attribute_multiplier_constant_(
            self.creator._container._impl._impl, NSLayoutAttributeBottom,
            NSLayoutRelationGreaterThanOrEqual if self.creator.resizeable else NSLayoutRelationEqual,
            self.creator._container._impl._impl, NSLayoutAttributeTop,
            1.0, 0
        )
        self.creator._container._impl._impl.addConstraint_(self._min_height_constraint)
    ...
    def show(self):
        ...
        self._min_width_constraint.constant = self.creator.content.layout.width
        self._min_height_constraint.constant = self.creator.content.layout.height

        # Do the first layout render.
        self.creator._container._update_layout(
